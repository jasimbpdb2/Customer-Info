...
if [ "$cygwin" = "true" ] || [ "$msys" = "true" ] ; then
    ...
    for i do
        EQUALS=`echo $i | sed -e ';s/=/\\=/g'`
        SLASH=`echo $i | sed -e ';s/\\//\\\\\\//g'`
        BSLASH=`echo $i | sed -e ';s/\\/\\\\/g'`
        if echo $i | grep -E -q "$OURCYGPATTERN"; then
            i=`echo $i | sed -e 's//\\\\\\//g'`
        elif echo $i | grep -E -q ":[A-Za-z]:[\\\\/]"; then
